...
Довольно часто значение столбца, который должен вернуть SQL-запрос, зависит от условий, которые нужно проверять для каждой строки. Для реализации подобного выбора используются выражения ***`CASE`***. Используя это выражение, можно реализовать условную логику ```if-then-else``` в операторе [***`SELECT`***](https://github.com/egorbeckish/ArtificialIntelligence/tree/main/SQL+PL-pgSQL/SQL/OperatorsStructure/SELECT).
...
---
> Выражение ***`CASE`*** может содержать другие выражения ***`CASE`***.
