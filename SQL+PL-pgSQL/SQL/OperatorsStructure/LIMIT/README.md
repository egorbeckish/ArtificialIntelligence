...
Используя предложения ***`LIMIT N`*** и ***`OFFSET M`***, можно ограничить количество строк в результате выполнения запроса. Если оператор [***`SELECT`***](https://github.com/egorbeckish/ArtificialIntelligence/tree/main/SQL+PL-pgSQL/SQL/OperatorsStructure/SELECT) содержит предложение ***`LIMIT N`***, то будет выводиться не более первых ***`N`*** строк результата. При использовании ***`LIMIT N`*** вместе с ***`OFFSET M`***. В этом случае сначала пропускаются первые ***`M`*** строк результата, после этого выводится ***`N`*** следующих строк.
