...
Результат выполнения оператора [***`SELECT`***](https://github.com/egorbeckish/ArtificialIntelligence/tree/main/SQL+PL-pgSQL/SQL/OperatorsStructure/SELECT) может быть упорядочен по значению одного или нескольких столбцов. Для этого служит предложение ***`ORDER BY`***.
...
> Сначала строки упорядочиваются по значению первого столбца. Строки, имеющие одинаковые значения первого столбца, упорядочиваются по значению второго и т.д. Для каждого столбца можно указать свой порядок сортировки.
