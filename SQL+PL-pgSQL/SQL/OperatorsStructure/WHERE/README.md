...
Для того чтобы выводить только те данные, которые удовлетворяют определенным условиям, оператор [***`SELECT`***](https://github.com/egorbeckish/ArtificialIntelligence/tree/main/SQL+PL-pgSQL/SQL/OperatorsStructure/SELECT) должен содержать предложение ***`WHERE`***, которое содержит условное выражение.
