...
Запрос 3. [Для сотрудника *employee_id=145* вывести период времени между датой приема на работу и сегодняшним днем](https://github.com/egorbeckish/ArtificialIntelligence/blob/main/SQL+PL-pgSQL/SQL/TypesAndFunctions/3.DateTimeType/AGE/sql/query3.sql)
```sql
SELECT
	AGE(hire_date)
FROM
...
```
> Результат, который возвращает функция **AGE()**, имеет тип [INTERVAL](https://github.com/egorbeckish/ArtificialIntelligence/blob/main/SQL+PL-pgSQL/SQL/TypesAndFunctions/3.DateTimeType/INTERVAL). Используя функцию [EXTRACT()](https://github.com/egorbeckish/ArtificialIntelligence/blob/main/SQL+PL-pgSQL/SQL/TypesAndFunctions/3.DateTimeType/EXTRACT), можно выбрать и использовать определенную часть этого результата.
